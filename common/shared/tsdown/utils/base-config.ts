...
import { createWorkspaceArtifactResolverPlugin } from '../plugins/workspace-artifact-resolver.ts';
import { hasSourceFiles } from './files.ts';

/**
 * Resolves extra bundler plugins based on the package source layout.
 */
export function createInputPlugins(packageDir: string): NonNullable<UserConfig['plugins']> {
    return [
        createWorkspaceArtifactResolverPlugin(packageDir),
